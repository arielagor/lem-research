...
## Judged-data findings (11/15 runs; LEM s2-s5 pending)

**Self-attribution correlates with survival, and cuts across arms.** Any-judge self-attribution by run: A all 0.00-0.03 (kept trading, all died); B bimodal — B-s1 0.40, B-s4 0.79, B-s5 0.73 (all withdrew, all won) but B-s2 0.00 (withdrew anyway) and B-s3 0.00 (kept trading, died); LEM-s1 0.48 (withdrew, won). So the mechanism is real but the discriminating variable is **withdraw-with-attribution vs keep-trading-without**, and B achieves it on 3 of 5 seeds. Whether LEM does it more *consistently* than B is the open question (needs LEM s2-s5). Strict (both-judge) rate stays near-zero except B-s5 0.51 and LEM-s1 0.13.

**H3 is INVERTED (important).** Pre-registration predicted LEM would maintain higher goal coherence. Instead coherence is **anti-correlated with survival**: stateless A scores m3=7 (ceiling) on every seed yet loses every seed — it keeps verbalizing the literal mission ("buy low, sell high, maximize net worth") while being exploited to death. The winning runs score m3=1-2 (B-s5 2, LEM-s1 2, B-s3 1) because the survival move — abandoning active trading — reads to the judges as drift from the stated trading mission. **Reflexivity makes the "coherent" behavior fatal.** This is a finding, not a failure of the arm: H3's premise (coherence is good) does not hold in a reflexive environment where the mission as literally stated is a trap. The paper's H3 section must be rewritten around this inversion, and the mission-statement design (rewarding trading) named as the reason.
